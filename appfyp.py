...
# PATHS
# ============================================================
CLS_PATH     = r"c:\Users\Saad Ahsin\Desktop\FYP-3 Reports\Swin\Models for Fyp\best_cls.pt"
GEN_PATH     = r"c:\Users\Saad Ahsin\Desktop\FYP-3 Reports\Swin\Models for Fyp\best_gen.pt"
BIOBART_NAME = "GanjinZero/biobart-base"
...
def load_models():
    ...
    cls_head     = ClassificationHead().to(device).eval()
    projection   = ProjectionMLP().to(device).eval()

    tokenizer    = BartTokenizer.from_pretrained(BIOBART_NAME)
    bart         = BartForConditionalGeneration.from_pretrained(BIOBART_NAME)
    decoder      = bart.model.decoder.to(device).float().eval()
    lm_head      = bart.lm_head.to(device).float().eval()
    embed_tokens = bart.model.shared.to(device).float().eval()
    del bart
    ...
    gen_ckpt = torch.load(GEN_PATH, map_location=device)
    swin.load_state_dict(gen_ckpt["swin"])
    projection.load_state_dict(gen_ckpt["projection"])
    decoder.load_state_dict(gen_ckpt["decoder"])
    lm_head.load_state_dict(gen_ckpt["lm_head"])

    return dict(swin=swin, cls_head=cls_head, projection=projection,
                decoder=decoder, lm_head=lm_head,
                embed_tokens=embed_tokens, tokenizer=tokenizer,
                device=device)
...
# GENERATE REPORT
# ============================================================
def generate_report(image, models, threshold=0.6):  # ← CHANGED to 0.6
    device = models["device"]

    img = transform(image).unsqueeze(0).to(device)

    feats = models["swin"].forward_features(img)
    if feats.dim() == 4:
        B, H, W, C = feats.shape
    ...
    with torch.no_grad():
        probs = torch.sigmoid(
            models["cls_head"](feats)).squeeze().cpu().numpy()

    # NEW: Different thresholds for different diseases
    detected = []
    for i, disease in enumerate(DISEASES):
        # Infiltration and Nodule need higher confidence (0.65)
        if disease in ["Infiltration", "Nodule"]:
            if probs[i] > 0.65:
                detected.append((disease, float(probs[i])))
        # All other diseases use normal threshold (0.6)
    ...
    # For report: use ONLY highest confidence disease
    if detected_names:
    ...
    if uploaded:
        if st.button("Generate Report", type="primary", use_container_width=True):
            with st.spinner("Analysing"):
                # Fixed threshold = 0.5 (no slider needed)
                detected_names, detected_probs, report, top_disease = \
                    generate_report(image, models, threshold=0.5)
            ...
            st.download_button(
                "📥 Download Report",
                data=f"PRIMARY: {top_disease}\n\nFINDINGS:\n" + 
                     (f"{'Normal' if not detected_names else chr(10).join(f'{n}: {p:.1%}' for n,p in zip(detected_names,detected_probs))}\n\nREPORT:\n{report}"),
...
st.markdown("---")
st.caption("⚠️ This is an AI FYP Prototype, Please Consult a Real Medical Professional After Vieweing Report.")